...
if ( ! function_exists( 'lunara_site_studio_workspace_config' ) ) {
	function lunara_site_studio_workspace_config( $surface_id, $surface ) {
		...
		$markers = array(
			'global-design' => array(),
			'homepage-structure' => array( 'hero', 'latest-reviews', 'pairing-desk', 'dispatch', 'oscar-picks', 'oscar-facts' ),
			'lunara-method' => array( 'pairing-desk' ),
		);
		return array(
...
if ( ! function_exists( 'lunara_site_studio_origin_key' ) ) {
	...
	function lunara_site_studio_workspace_urls( $surface ) {
		...
		$admin = admin_url( '/' ); $home = home_url( '/' ); $preview = home_url( $surface['preview_route'] );
		foreach ( array( $admin, $home, $preview ) as $url ) { if ( ! is_string( $url ) || '' === lunara_site_studio_origin_key( $url ) || null !== wp_parse_url( $url, PHP_URL_QUERY ) ) { return array(); } }
		...
		$preview_path = (string) wp_parse_url( $preview, PHP_URL_PATH ); if ( '' === $preview_path ) { $preview_path = '/'; }
		if ( ! lunara_site_studio_same_origin( $admin, $home ) || ! lunara_site_studio_same_origin( $home, $preview ) || $preview_path !== $expected_path ) { return array(); }
		return array( 'admin' => $admin, 'home' => $home, 'preview' => $preview );
	}
	function lunara_site_studio_safe_admin_destination( $url ) {
		$admin = admin_url( '/' ); if ( ! is_string( $url ) || ! lunara_site_studio_same_origin( $url, $admin ) ) { return ''; }
...
if ( ! function_exists( 'lunara_site_studio_render_details_open' ) ) {
	function lunara_site_studio_render_details_open( $section, $label, $open = false ) { echo '<details' . ( $open ? ' open' : '' ) . ' class="lunara-site-studio-details" data-section="' . esc_attr( $section ) . '"><summary>' . esc_html( $label ) . '</summary><div class="lunara-site-studio-details-body">'; }
...
if ( ! function_exists( 'lunara_render_site_studio_page' ) ) {
	function lunara_render_site_studio_page() {
		...
		$active = $surfaces[ $active_id ]; $pilots = array( 'global-design', 'homepage-structure', 'lunara-method' ); $availability = lunara_site_studio_surface_availability( $active ); $adapter = null; $state = null; $revisions = array(); $workspace_error = false; $workspace_urls = lunara_site_studio_workspace_urls( $active ); $workspace_config = lunara_site_studio_workspace_config( $active_id, $active ); $transport_safe = ! empty( $workspace_urls ) && lunara_site_studio_workspace_config_is_safe( $workspace_config ); $usable = ! empty( $availability['available'] ) && in_array( $active_id, $pilots, true ) && $transport_safe;
		if ( $usable ) { $adapter = lunara_site_studio_get_adapter( $active_id ); $state = lunara_site_studio_call_adapter( $adapter, 'read_state' ); $listed = lunara_site_studio_call_adapter( $adapter, 'list_revisions' ); if ( ! is_wp_error( $state ) ) { $state = lunara_site_studio_project_state( $active_id, $state ); } if ( is_wp_error( $adapter ) || is_wp_error( $state ) || is_wp_error( $listed ) || ! is_array( $state ) || ! is_array( $listed ) ) { $usable = false; $workspace_error = true; } else { $revisions = lunara_site_studio_safe_revisions( $listed ); } } if ( ! $transport_safe && ! empty( $availability['available'] ) && in_array( $active_id, $pilots, true ) ) { $workspace_error = true; }
		?>
		<div class="wrap lunara-site-studio" data-lunara-site-studio data-surface="<?php echo esc_attr( $active_id ); ?>" data-workspace-state="recovery" data-dirty="false">
			...
			<?php foreach ( $surfaces as $id => $surface ) : $status = lunara_site_studio_surface_availability( $surface ); $section_terms = array(); foreach ( $surface['sections'] as $section ) { $section_terms[] = $section; $section_terms[] = ucwords( str_replace( '-', ' ', $section ) ); } $search = implode( ' ', array_merge( array( $surface['label'], $surface['group'], $surface['description'] ), $surface['aliases'], $section_terms ) ); ?>
				<?php $icons = array( 'global-design' => 'admin-customizer', 'homepage-structure' => 'admin-home', 'lunara-method' => 'heart', 'reviews-archive' => 'star-filled', 'journal-archive' => 'edit-page', 'oscars-portal' => 'awards' ); $method_thumb = 'lunara-method' === $id && 'lunara-method' === $active_id && $usable ? lunara_site_studio_safe_local_thumbnail( wp_get_attachment_image_url( absint( $state['backdrop_id'] ), 'thumbnail' ) ) : ''; $card_url = lunara_site_studio_safe_admin_destination( lunara_site_studio_admin_url( $id ) ); ?><a class="lunara-site-studio-card lunara-site-studio-card--<?php echo esc_attr( $id ); ?><?php echo $id === $active_id ? ' is-active' : ''; ?>" data-lunara-surface-card data-surface="<?php echo esc_attr( $id ); ?>" data-search-index="<?php echo esc_attr( strtolower( $search ) ); ?>" href="<?php echo esc_url( $card_url ); ?>"<?php echo $id === $active_id ? ' aria-current="page"' : ''; ?>><?php if ( $method_thumb ) : ?><img class="lunara-site-studio-card-thumb" src="<?php echo esc_url( $method_thumb ); ?>" alt="" loading="lazy" decoding="async" /><?php else : ?><span class="dashicons dashicons-<?php echo esc_attr( isset( $icons[ $id ] ) ? $icons[ $id ] : 'admin-generic' ); ?>" aria-hidden="true"></span><?php endif; ?><small><?php echo esc_html( $surface['group'] ); ?></small><strong><?php echo esc_html( $surface['label'] ); ?></strong><span><?php echo esc_html( $surface['description'] ); ?></span><em><?php echo esc_html( ! empty( $status['available'] ) ? __( 'Available', 'lunara-film' ) : __( 'Unavailable', 'lunara-film' ) ); ?></em></a>
			...
			<div class="lunara-site-studio-workspace">
				<aside class="lunara-site-studio-section-rail" aria-label="<?php echo esc_attr__( 'Site sections', 'lunara-film' ); ?>"><h2><?php esc_html_e( 'Sections', 'lunara-film' ); ?></h2><?php if ( 'homepage-structure' === $active_id ) { lunara_site_studio_render_home_order( $state ); } else { ?><ol><?php foreach ( $active['sections'] as $section ) : ?><li><?php echo esc_html( ucwords( str_replace( '-', ' ', $section ) ) ); ?></li><?php endforeach; ?></ol><?php } ?></aside>
