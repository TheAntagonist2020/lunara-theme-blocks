...
require_once __DIR__ . '/site-studio-navigation.php';
...
if ( ! function_exists( 'lunara_site_studio_workspace_surface' ) ) {
	function lunara_site_studio_workspace_surface( $surfaces = null ) {
		...
		if ( isset( $surfaces['homepage-structure'] ) ) { return 'homepage-structure'; }
...
if ( ! function_exists( 'lunara_site_studio_workspace_editable_surfaces' ) ) {
	...
	function lunara_site_studio_workspace_editable_surfaces() {
		return array( 'home-oscar-picks', 'home-oscar-facts', 'global-design', 'homepage-structure', 'lunara-method', 'reviews-archive', 'review-single', 'journal-archive', 'journal-single', 'utility-search', 'utility-404', 'site-footer', 'hero-carousel', 'journal-carousel', 'reviews-carousel', 'oscars-portal', 'oscars-ledger', 'debrief-method' );
	}
}
...
if ( ! function_exists( 'lunara_site_studio_workspace_config' ) ) {
	/** Site-prefixed versions of the provider's four fixed preview destinations. */
	function lunara_site_studio_workspace_ledger_routes() {
		if ( ! function_exists( 'lunara_site_studio_oscars_ledger_preview_routes' ) ) { return array(); }
		return array_map( static function ( $route ) { return (string) wp_parse_url( home_url( $route ), PHP_URL_PATH ); }, lunara_site_studio_oscars_ledger_preview_routes() );
	}
	function lunara_site_studio_workspace_config( $surface_id, $surface ) {
		...
		$config = array(
			...
			'strings' => array(
				'live' => __( 'Live settings loaded.', 'lunara-film' ), 'dirty' => __( 'Unsaved changes.', 'lunara-film' ), 'previewCurrent' => __( 'Preview is current.', 'lunara-film' ), 'previewStale' => __( 'Preview is out of date.', 'lunara-film' ), 'saving' => __( 'Applying changes…', 'lunara-film' ), 'saved' => __( 'Changes applied.', 'lunara-film' ), 'restored' => __( 'Revision restored.', 'lunara-film' ), 'failed' => __( 'The request could not be completed. Your changes are still here.', 'lunara-film' ),
...
if ( ! function_exists( 'lunara_site_studio_origin_key' ) ) {
	...
	function lunara_site_studio_workspace_config_is_safe( $config ) {
		...
		if ( isset( $config['surface'] ) && 'oscars-ledger' === $config['surface'] ) {
		...
		if ( 'journal-single' === $config['surface'] ) { $preview_meta['route'] = lunara_site_studio_journal_single_preview_route(); }
...
if ( ! function_exists( 'lunara_enqueue_site_studio_assets' ) ) {
	function lunara_enqueue_site_studio_assets( $hook ) {
		...
		$asset = lunara_resolve_theme_asset( 'assets/css/lunara-site-studio.css' );
		if ( ! empty( $asset['uri'] ) ) { wp_enqueue_style( 'lunara-site-studio', $asset['uri'], array( 'dashicons' ), function_exists( 'lunara_theme_asset_version' ) ? lunara_theme_asset_version( $asset['path'] ) : false ); }
		...
		if ( $uses_editor_controls ) {
			...
			if ( ! empty( $asset['uri'] ) ) { wp_enqueue_style( 'lunara-editor-controls', $asset['uri'], array( 'lunara-site-studio' ), function_exists( 'lunara_theme_asset_version' ) ? lunara_theme_asset_version( $asset['path'] ) : false ); }
			$asset = lunara_resolve_theme_asset( 'assets/js/lunara-editor-controls.js' );
			if ( ! empty( $asset['uri'] ) ) { wp_enqueue_script( 'lunara-editor-controls', $asset['uri'], array(), function_exists( 'lunara_theme_asset_version' ) ? lunara_theme_asset_version( $asset['path'] ) : false, true ); }
			$script_dependencies[] = 'lunara-editor-controls';
		}
		if ( $is_carousel ) {
			$asset = lunara_resolve_theme_asset( 'assets/css/lunara-site-studio-carousels.css' );
			if ( ! empty( $asset['uri'] ) ) { wp_enqueue_style( 'lunara-site-studio-carousels', $asset['uri'], array( 'lunara-editor-controls' ), function_exists( 'lunara_theme_asset_version' ) ? lunara_theme_asset_version( $asset['path'] ) : false ); }
			$asset = lunara_resolve_theme_asset( 'assets/js/lunara-site-studio-carousels.js' );
			if ( ! empty( $asset['uri'] ) ) { wp_enqueue_script( 'lunara-site-studio-carousels', $asset['uri'], array( 'lunara-editor-controls' ), function_exists( 'lunara_theme_asset_version' ) ? lunara_theme_asset_version( $asset['path'] ) : false, true ); }
			$script_dependencies = array( 'lunara-site-studio-carousels' );
		}
		if ( 'lunara-method' === $surface_id ) {
			$asset = lunara_resolve_theme_asset( 'assets/js/lunara-site-studio-method.js' );
			if ( ! empty( $asset['uri'] ) ) { wp_enqueue_script( 'lunara-site-studio-method', $asset['uri'], array( 'lunara-editor-controls' ), function_exists( 'lunara_theme_asset_version' ) ? lunara_theme_asset_version( $asset['path'] ) : false, true ); }
			$script_dependencies = array( 'lunara-site-studio-method' );
		}
		if ( $uses_home_oscars ) {
			$asset = lunara_resolve_theme_asset( 'assets/js/lunara-site-studio-home-oscars.js' );
...
if ( ! function_exists( 'lunara_site_studio_render_field' ) ) {
	function lunara_site_studio_render_field( $path, $label, $value, $type = 'text', $error_key = '', $effective_id = '' ) {
		...
		echo '<span class="description" id="' . esc_attr( $id ) . '-help">' . esc_html__( 'Changes stay local until Preview changes or Apply changes.', 'lunara-film' ) . '</span><span class="lunara-site-studio-error" id="' . esc_attr( $id ) . '-error" hidden></span></div>';
	}
}
...
if ( ! function_exists( 'lunara_site_studio_control_label' ) ) {
	...
	function lunara_site_studio_choice_label( $choice ) { return 'legacy' === $choice ? __( 'Keep current selection', 'lunara-film' ) : ucwords( str_replace( '-', ' ', $choice ) ); }
}
...
if ( ! function_exists( 'lunara_site_studio_render_mod_surface_inspector' ) ) {
	function lunara_site_studio_render_mod_surface_inspector( $surface_id, $state, $revisions, $classic_url ) {
		...
		} elseif ( 'journal-single' === $surface_id ) {
			$spec = lunara_site_studio_journal_single_spec();
			lunara_site_studio_render_details_open( 'content', __( 'Headline and byline', 'lunara-film' ), true, array( 'hero', 'article' ) );
			echo '<p>' . esc_html__( 'These settings apply to every Journal article. The preview uses the newest published article.', 'lunara-film' ) . '</p>';
			lunara_site_studio_render_spec_paths( $state, $spec, array( 'hero.title_size', 'metadata.show_byline', 'metadata.show_date', 'metadata.show_reading_time' ) );
			lunara_site_studio_render_details_close();
			lunara_site_studio_render_details_open( 'artwork', __( 'Hero image framing', 'lunara-film' ), true, array( 'hero' ) );
			$article = lunara_site_studio_journal_single_preview_article();
			$image_url = $article && function_exists( 'get_post_thumbnail_id' ) ? wp_get_attachment_image_url( get_post_thumbnail_id( $article['id'] ), 'full' ) : '';
			echo '<p>' . esc_html__( 'Fill the frame and move the focal point, or show the full image. Use the desktop and mobile preview buttons to check the result.', 'lunara-film' ) . '</p><div data-journal-image-framing data-image-url="' . esc_url( $image_url ? $image_url : '' ) . '"></div><div data-journal-framing-fields>';
			lunara_site_studio_render_spec_paths( $state, $spec, array( 'hero.image_fit', 'hero.image_position_x', 'hero.image_position_y' ) );
			echo '</div>';
			lunara_site_studio_render_details_close();
			lunara_site_studio_render_details_open( 'article-editor', __( 'Article text, images and gallery', 'lunara-film' ), true, array( 'article', 'gallery' ) );
			echo '<p>' . esc_html__( 'Change each article’s text, Featured image, image description and Media Gallery in its article editor. This layout editor keeps those article settings intact.', 'lunara-film' ) . '</p>';
			if ( $article && current_user_can( 'edit_post', $article['id'] ) ) { echo '<a class="button" data-workspace-navigation href="' . esc_url( admin_url( 'post.php?post=' . $article['id'] . '&action=edit' ) ) . '">' . esc_html__( 'Edit the preview article and its images', 'lunara-film' ) . '</a>'; }
			echo '<a class="button" data-workspace-navigation href="' . esc_url( admin_url( 'edit.php?post_type=journal' ) ) . '">' . esc_html__( 'Choose another Journal article to edit', 'lunara-film' ) . '</a>';
			lunara_site_studio_render_details_close();
...
if ( ! function_exists( 'lunara_site_studio_render_archive_order' ) ) {
	function lunara_site_studio_render_archive_order( $state, $derived = array() ) {
...
if ( ! function_exists( 'lunara_site_studio_render_archive_inspector' ) ) {
	function lunara_site_studio_render_archive_inspector( $surface_id, $state, $revisions, $classic_url ) {
		...
		foreach ( $specs as $path => $spec ) { lunara_site_studio_render_control( $path, lunara_site_studio_state_path_value( $state, $path ), $spec ); }
		lunara_site_studio_render_details_close();
...
if ( ! function_exists( 'lunara_site_studio_render_oscars_portal_inspector' ) ) {
	function lunara_site_studio_render_oscars_portal_inspector( $state, $revisions ) {
		lunara_site_studio_render_details_open( 'essentials', __( 'Content', 'lunara-film' ), true, array( 'hero', 'doors', 'spotlights', 'titles', 'research', 'linked-reviews', 'deep-cuts' ) );
		foreach ( lunara_oscars_portal_studio_identity_specs() as $field => $spec ) { lunara_site_studio_render_control( 'identity.' . $field, $state['identity'][ $field ], array( 'type' => 'text', 'max_length' => $spec['max'] ), $spec['label'] ); }
		lunara_site_studio_render_details_close();
		lunara_site_studio_render_details_open( 'hero-buttons', __( 'Hero buttons', 'lunara-film' ), false, array( 'hero' ) );
		echo '<p>' . esc_html__( 'These labels appear on the three links in the Oscars hero. Their destinations remain the latest ceremony, full ledger and category archive. Leave a label blank to use its default text.', 'lunara-film' ) . '</p>';
		$button_labels = array( 'ceremony' => __( 'Latest ceremony button text', 'lunara-film' ), 'ledger' => __( 'Full ledger button text', 'lunara-film' ), 'categories' => __( 'Category archive button text', 'lunara-film' ) );
		foreach ( lunara_oscars_portal_studio_button_specs() as $field => $spec ) { lunara_site_studio_render_control( 'buttons.' . $field, $state['buttons'][ $field ], array( 'type' => 'text', 'max_length' => $spec['max'], 'help' => sprintf( __( 'Leave blank to use “%s”.', 'lunara-film' ), $spec['default'] ) ), $button_labels[ $field ] ); }
		lunara_site_studio_render_details_close();
		lunara_site_studio_render_details_open( 'quick-start', __( 'Quick Start', 'lunara-film' ), false, array( 'doors' ) );
		echo '<p>' . esc_html__( 'These four cards appear in the Quick Start section of the Oscars page. Each keeps its fixed position and artwork. Hiding a card retains its text and destination. The section heading is in Content; section visibility is in the section list.', 'lunara-film' ) . '</p>';
		$card_labels = array( 'ceremonies' => __( 'Ceremonies', 'lunara-film' ), 'categories' => __( 'Categories', 'lunara-film' ), 'ledger' => __( 'Full ledger', 'lunara-film' ), 'method' => __( 'Ledger method', 'lunara-film' ) );
		$destinations = array( 'ceremonies' => __( 'the ceremony archive', 'lunara-film' ), 'categories' => __( 'the category archive', 'lunara-film' ), 'ledger' => __( 'the full ledger table', 'lunara-film' ), 'method' => __( 'the ledger method page', 'lunara-film' ) );
		foreach ( lunara_oscars_portal_studio_quick_start_specs() as $slot => $spec ) {
			lunara_site_studio_render_details_open( 'quick-start-' . $slot, sprintf( __( 'Card %1$d: %2$s', 'lunara-film' ), $spec['slot'], $card_labels[ $slot ] ) );
			$prefix = 'quick_start.' . $slot . '.'; $card = $state['quick_start'][ $slot ];
			lunara_site_studio_render_control( $prefix . 'enabled', $card['enabled'], array( 'type' => 'bool', 'help' => __( 'Hidden cards keep their saved text and destination.', 'lunara-film' ) ), __( 'Show this card', 'lunara-film' ) );
			lunara_site_studio_render_control( $prefix . 'kicker', $card['kicker'], array( 'type' => 'text', 'max_length' => 140, 'help' => sprintf( __( 'Leave blank to use “%s”.', 'lunara-film' ), $spec['kicker'] ) ), __( 'Kicker', 'lunara-film' ) );
			lunara_site_studio_render_control( $prefix . 'title', $card['title'], array( 'type' => 'text', 'max_length' => 220, 'help' => sprintf( __( 'Leave blank to use “%s”.', 'lunara-film' ), $spec['title'] ) ), __( 'Heading', 'lunara-film' ) );
			lunara_site_studio_render_control( $prefix . 'copy', $card['copy'], array( 'type' => 'textarea', 'max_length' => 600, 'help' => __( 'Optional description below the card heading.', 'lunara-film' ) ), __( 'Description', 'lunara-film' ) );
			lunara_site_studio_render_control( $prefix . 'url', $card['url'], array( 'type' => 'text', 'max_length' => 2048, 'help' => sprintf( __( 'Leave blank to follow %s automatically. Or enter an http/https address or a site path beginning with /.', 'lunara-film' ), $destinations[ $slot ] ) ), __( 'Destination', 'lunara-film' ) );
			lunara_site_studio_render_details_close();
		}
		lunara_site_studio_render_details_close();
...
if ( ! function_exists( 'lunara_site_studio_render_method_inspector' ) ) {
	function lunara_site_studio_render_method_inspector( $state, $revisions, $classic_url ) {
		lunara_site_studio_render_details_open( 'essentials', __( 'Content', 'lunara-film' ), true ); lunara_site_studio_render_field( 'kicker', __( 'Kicker', 'lunara-film' ), $state['kicker'] ); lunara_site_studio_render_field( 'title', __( 'Title', 'lunara-film' ), $state['title'] ); lunara_site_studio_render_field( 'copy', __( 'Supporting copy', 'lunara-film' ), $state['copy'], 'textarea' ); lunara_site_studio_render_details_close();
		lunara_site_studio_render_details_open( 'fine-tune', __( 'Stories and images', 'lunara-film' ) );
		echo '<div data-method-editor><label>Featured Review <select data-method-mode data-error-key="review_mode" aria-describedby="lunara-method-mode-error"><option value="automatic">Automatic — newest eligible Review</option><option value="manual">Manual selection</option></select></label><span id="lunara-method-mode-error" class="lunara-site-studio-error" hidden></span><p>Automatic selects the newest of the latest 20 published Reviews with a pairing filled. Switching modes retains your manual selection.</p><div data-method-manual><label>Search published Reviews <input type="search" data-method-search></label><button type="button" data-method-search-button>Search</button><div data-method-results aria-live="polite"></div><button type="button" data-method-clear>Clear selection</button></div><div data-method-selected data-error-key="review_id" tabindex="-1" aria-describedby="lunara-method-review-error" aria-live="polite"></div><span id="lunara-method-review-error" class="lunara-site-studio-error" hidden></span><button type="button" data-method-retry hidden>Retry details</button><div data-error-key="backdrop_id" tabindex="-1" aria-describedby="lunara-method-image-error"><div data-method-image data-error-key="backdrop" tabindex="-1" aria-describedby="lunara-method-framing-error"></div><span id="lunara-method-image-error" class="lunara-site-studio-error" hidden></span><span id="lunara-method-framing-error" class="lunara-site-studio-error" hidden></span></div></div>';
		lunara_site_studio_render_details_close();
		lunara_site_studio_render_details_open( 'mobile', __( 'Mobile', 'lunara-film' ) ); echo '<p>' . esc_html__( 'The decorative backdrop is hidden at widths up to 820px. Review selection and copy still apply on mobile. Use the desktop preview to check backdrop framing.', 'lunara-film' ) . '</p>'; lunara_site_studio_render_details_close();
...
if ( ! function_exists( 'lunara_site_studio_render_revisions' ) ) {
	...
	function lunara_site_studio_render_revisions( $revisions ) {
		echo '<details class="lunara-site-studio-details" data-section="revision-history" data-revision-history><summary data-revision-summary>' . esc_html__( 'History', 'lunara-film' ) . '</summary><div class="lunara-site-studio-details-body"><div data-revision-list>';
...
if ( ! function_exists( 'lunara_render_site_studio_page' ) ) {
	function lunara_render_site_studio_page() {
		...
		<div class="wrap lunara-site-studio" data-lunara-site-studio data-surface="<?php echo esc_attr( $active_id ); ?>" data-workspace-state="recovery" data-dirty="false">
			<header class="lunara-site-studio-header"><div><p class="lunara-site-studio-kicker"><?php esc_html_e( 'Lunara · Your site', 'lunara-film' ); ?></p><h1><?php esc_html_e( 'Site Studio', 'lunara-film' ); ?></h1><p><?php esc_html_e( 'Choose a page, shape its presentation, then preview and apply your changes.', 'lunara-film' ); ?></p></div></header>
			<?php lunara_site_studio_render_page_navigation( $surfaces, $active_id ); ?>
			<details class="lunara-site-studio-map" data-studio-tool-directory><summary id="lunara-site-map-heading"><?php esc_html_e( 'All editors and tools', 'lunara-film' ); ?></summary><div class="lunara-site-studio-directory-body"><label for="lunara-site-studio-search"><?php esc_html_e( 'Search destinations', 'lunara-film' ); ?></label><input id="lunara-site-studio-search" type="search" data-lunara-surface-search /><p data-lunara-search-count aria-live="polite"><?php echo esc_html( sprintf( __( '%d destinations', 'lunara-film' ), count( $surfaces ) ) ); ?></p><div class="lunara-site-studio-map-grid">
			...
			<div class="lunara-site-studio-workspace">
				<aside class="lunara-site-studio-section-rail" aria-label="<?php echo esc_attr__( 'Site sections', 'lunara-film' ); ?>"><h2><?php esc_html_e( 'Sections', 'lunara-film' ); ?></h2><?php if ( 'homepage-structure' === $active_id ) { lunara_site_studio_render_home_order( $state ); } elseif ( 'oscars-portal' === $active_id ) { lunara_site_studio_render_archive_order( $state, array( 'board' => 'Shown when board content is available', 'navigator' => $state['section_visibility']['doors'] ? 'Visible with doors' : 'Hidden with doors' ) ); } elseif ( in_array( $active_id, array( 'reviews-archive', 'journal-archive' ), true ) ) { lunara_site_studio_render_archive_order( $state ); } else { ?><ol><?php foreach ( $active['sections'] as $section ) : ?><li data-section-control="<?php echo esc_attr( $section ); ?>" tabindex="-1"><?php echo esc_html( ucwords( str_replace( '-', ' ', $section ) ) ); ?></li><?php endforeach; ?></ol><?php } ?></aside>
				...
				<aside class="lunara-site-studio-inspector" aria-label="<?php echo esc_attr__( 'Settings inspector', 'lunara-film' ); ?>" aria-busy="false"><h2><?php echo esc_html( $active['label'] ); ?></h2><?php if ( in_array( $active_id, array( 'home-oscar-picks', 'home-oscar-facts' ), true ) ) { lunara_site_studio_render_home_oscars_inspector( $active_id, $state, $revisions ); } elseif ( in_array( $active_id, array( 'hero-carousel', 'journal-carousel', 'reviews-carousel' ), true ) ) { lunara_site_studio_render_carousel_inspector( $active_id, $state, $revisions ); } elseif ( 'global-design' === $active_id ) { lunara_site_studio_render_global_inspector( $state, $revisions, $active['classic_url'] ); } elseif ( 'homepage-structure' === $active_id ) { lunara_site_studio_render_home_inspector( $state, $revisions, $active['classic_url'] ); } elseif ( 'lunara-method' === $active_id ) { lunara_site_studio_render_method_inspector( $state, $revisions, $active['classic_url'] ); } elseif ( 'oscars-ledger' === $active_id ) { lunara_site_studio_render_oscars_ledger_inspector( $state, $revisions, $active['classic_url'] ); } elseif ( 'debrief-method' === $active_id ) { lunara_site_studio_render_debrief_method_inspector( $state, $revisions ); } elseif ( 'oscars-portal' === $active_id ) { lunara_site_studio_render_oscars_portal_inspector( $state, $revisions ); } elseif ( in_array( $active_id, array( 'reviews-archive', 'journal-archive' ), true ) ) { lunara_site_studio_render_archive_inspector( $active_id, $state, $revisions, $active['classic_url'] ); } else { lunara_site_studio_render_mod_surface_inspector( $active_id, $state, $revisions, $active['classic_url'] ); } ?><div class="lunara-site-studio-actions"><button type="button" class="button" data-action="preview" disabled><?php esc_html_e( 'Preview changes', 'lunara-film' ); ?></button><button type="button" class="button button-primary" data-action="save" disabled><?php esc_html_e( 'Apply changes', 'lunara-film' ); ?></button><button type="button" class="button" data-action="discard" disabled><?php esc_html_e( 'Discard changes', 'lunara-film' ); ?></button></div><p class="lunara-site-studio-status" data-workspace-status aria-live="polite"><?php esc_html_e( 'Loading editor…', 'lunara-film' ); ?></p></aside>
			</div><?php endif; ?>
		</div><?php
	}
}
