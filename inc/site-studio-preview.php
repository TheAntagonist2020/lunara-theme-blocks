...
if ( ! function_exists( 'lunara_site_studio_preview_pilots' ) ) {
	function lunara_site_studio_preview_pilots() {
		return array(
			'global-design' => array( 'owner' => 'theme:global-design', 'query' => 'lunara_global_design_preview', 'markers' => array() ),
			'homepage-structure' => array( 'owner' => 'theme:homepage-structure', 'query' => 'lunara_homepage_preview', 'markers' => array( 'hero', 'latest-reviews', 'pairing-desk', 'dispatch', 'oscar-picks', 'oscar-facts' ) ),
			'lunara-method' => array( 'owner' => 'theme:lunara-method', 'query' => 'lunara_method_preview', 'markers' => array( 'pairing-desk' ) ),
		);
	}
}
...
if ( ! function_exists( 'lunara_site_studio_preview_exact_query' ) ) {
	function lunara_site_studio_preview_exact_query( $raw_query ) {
		$segments = explode( '&', (string) $raw_query ); if ( ! is_array( $_GET ) || 2 !== count( $_GET ) || 2 !== count( $segments ) || in_array( '', $segments, true ) ) { return false; }
		$values = array();
...
if ( ! function_exists( 'lunara_site_studio_preview_request_origin' ) ) {
	...
	function lunara_site_studio_preview_request_path() {
		...
		return $request_query === $raw_query && $request_path === $home_path;
	}
}
...
if ( ! function_exists( 'lunara_site_studio_resolve_private_preview' ) ) {
	function lunara_site_studio_resolve_private_preview() {
		...
		$values = lunara_site_studio_preview_exact_query( isset( $_SERVER['QUERY_STRING'] ) ? $_SERVER['QUERY_STRING'] : '' ); if ( ! is_array( $values ) ) { return false; }
		...
		if ( '' === $selected || 2 !== count( $values ) || ! lunara_site_studio_preview_uuid( $values[ $pilot['query'] ] ) ) { return false; }
		$surface = lunara_site_studio_get_surface( $selected ); $surface_keys = array( 'id', 'owner', 'supports_preview', 'preview_route', 'preview_query_arg', 'capability' );
		if ( ! is_array( $surface ) || array() !== array_diff( $surface_keys, array_keys( $surface ) ) || ! is_string( $surface['id'] ) || ! is_string( $surface['owner'] ) || ! is_bool( $surface['supports_preview'] ) || ! is_string( $surface['preview_route'] ) || ! is_string( $surface['preview_query_arg'] ) || ! is_string( $surface['capability'] ) || '' === $surface['capability'] || $selected !== $surface['id'] || $pilot['owner'] !== $surface['owner'] || true !== $surface['supports_preview'] || '/' !== $surface['preview_route'] || $pilot['query'] !== $surface['preview_query_arg'] ) { return false; }
		if ( ! lunara_site_studio_preview_request_path() || ! is_front_page() ) { return false; }
		$front_page_id = 'page' === get_option( 'show_on_front' ) ? absint( get_option( 'page_on_front' ) ) : 0; if ( ! $front_page_id || absint( get_queried_object_id() ) !== $front_page_id ) { return false; }
		...
		$state = lunara_site_studio_get_private_preview( $selected, $pilot['owner'], '/', $values[ $pilot['query'] ] ); if ( ! is_array( $state ) ) { return false; } $validated = lunara_site_studio_call_adapter( $adapter, 'validate_state', array( $state ) ); if ( is_wp_error( $validated ) || ! is_array( $validated ) || $validated !== $state ) { return false; } $projected = lunara_site_studio_project_state( $selected, $validated ); if ( is_wp_error( $projected ) || ! is_array( $projected ) || $projected !== $validated || ! lunara_site_studio_preview_install_state( $selected, $projected, $front_page_id ) ) { return false; }
