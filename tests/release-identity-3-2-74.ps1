...
$exactVersionHeaders = @($styleLines | Where-Object { $_ -ceq 'Version: 3.2.74' })
Assert-Contract ($versionHeaderLines.Count -eq 1 -and $exactVersionHeaders.Count -eq 1) `
    'style.css must contain exactly one exact Version: 3.2.74 header.'

$priorVersion = @('3', '2', '73') -join '.'
...
$changelogHeading = "## 2026-09-13 $releaseSeparator Theme 3.2.74 Oscars Navigation Editor"
...
Assert-Contract ($changelogHeadingCount -eq 1) 'The 3.2.74 changelog heading must exist exactly once.'
...
# 2026-08-31 change that removed it). What is asserted instead: the 3.2.74
...
Assert-Contract ($changelogHeadingIndex -ge 0 -and $newerThemeHeadings.Count -eq 0) `
    'The 3.2.74 changelog entry must be the newest theme release entry; only plugin-only entries may sit above it.'
$changelogEntry = Get-TopEntry -Text $changelog -Heading $changelogHeading
foreach ($term in @('Oscars', 'Hero', 'Quick Start', 'Preview', 'History', 'mobile')) {
    Assert-Contract ($changelogEntry.Contains($term)) "The current changelog must cover $term."
}

$sessionHeading = "## 2026-09-13 $releaseSeparator Theme $priorVersion acceptance and 3.2.74 Oscars navigation"
...
Assert-Contract ($sessionHeadingCount -eq 1) 'The final 3.2.74 local-candidate session heading must exist exactly once.'
...
Assert-Contract ($sessionEntry -match '(?s)No agent deployment or manual cache purge occurred\. Theme 3\.2\.74 is not\s+deployed; its live canary is pending\.') `
...
if ($script:Failures.Count -gt 0) {
    ...
    throw "Theme 3.2.74 release identity contract failed:`n$($details -join "`n")"
}

Write-Host 'Theme 3.2.74 release identity contract passed: exact stylesheet identity, stale-version census, deploy exclusions, and intact local-only release records.'
