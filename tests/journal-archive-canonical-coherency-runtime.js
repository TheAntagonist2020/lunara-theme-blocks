...
assert.notEqual('3.2.48', '3.2.63', 'A version sweep must never collapse the stale fixture onto the expected version.');
...
const missingBinding = analyzeJournalCanonicalCoherency({
	...
	finalUrl: CANONICAL_JOURNAL_URL,
	statusCode: 200,
	html: candidateHtml().replace(/ data-lunara-theme-version="3\.2\.63"/, ''),
	expectedVersion: '3.2.63',
});
...
assert.equal(analyzeJournalCanonicalCoherency({ url: CANONICAL_JOURNAL_URL, finalUrl: CANONICAL_JOURNAL_URL, statusCode: 200, html: '', expectedVersion: '3.2.63' }).coherent, false, 'An empty response must fail closed.');
assert.equal(analyzeJournalCanonicalCoherency({ url: CANONICAL_JOURNAL_URL, finalUrl: CANONICAL_JOURNAL_URL, statusCode: 200, html: '<html><head></head>no body tag', expectedVersion: '3.2.63' }).coherent, false, 'A response with no <body> must fail closed.');
assert.equal(analyzeJournalCanonicalCoherency({ url: CANONICAL_JOURNAL_URL, finalUrl: CANONICAL_JOURNAL_URL, statusCode: 200, html: null, expectedVersion: '3.2.63' }).coherent, false, 'A null body must fail closed.');
...
const cliCoherentReplay = runCli(['--expected-version', '3.2.63', '--replay-html-file', coherentFixture, '--replay-status', '200', '--replay-final-url', CANONICAL_JOURNAL_URL]);
...
const cliLegacyReplay = runCli(['--expected-version', '3.2.63', '--replay-html-file', legacyFixture, '--replay-status', '200', '--replay-final-url', CANONICAL_JOURNAL_URL]);
...
const cliAuthReplay = runCli(['--expected-version', '3.2.63', '--replay-html-file', coherentFixture, '--replay-status', '200', '--replay-final-url', 'https://admin@lunarafilm.com/journal/']);
...
const cliQueryReplay = runCli(['--expected-version', '3.2.63', '--replay-html-file', coherentFixture, '--replay-status', '200', '--replay-final-url', 'https://lunarafilm.com/journal/?nocache=1']);
assert.equal(cliQueryReplay.status, INCOHERENT_EXIT, 'A cache-busted final URL must fail the CLI even with coherent HTML.');

const cliStatusReplay = runCli(['--expected-version', '3.2.63', '--replay-html-file', coherentFixture, '--replay-status', '503', '--replay-final-url', CANONICAL_JOURNAL_URL]);
assert.equal(cliStatusReplay.status, INCOHERENT_EXIT, 'A non-200 replay status must fail the CLI.');

const cliCards = runCli(['--expected-version', '3.2.63', '--expected-cards', '12', '--replay-html-file', coherentFixture, '--replay-status', '200', '--replay-final-url', CANONICAL_JOURNAL_URL]);
...
assert.equal(runCli(['--expected-version', '3.2.63', '--bogus-flag', 'x']).status, USAGE_ERROR_EXIT, 'An unknown flag must be a usage error.');
assert.equal(runCli(['--expected-version', '3.2.63', '--replay-html-file', coherentFixture, '--replay-status', '200']).status, USAGE_ERROR_EXIT, 'Replay without an explicit --replay-final-url must be refused, never defaulted.');
assert.equal(runCli(['--expected-version', '3.2.63', '--replay-html-file', coherentFixture, '--replay-final-url', CANONICAL_JOURNAL_URL]).status, USAGE_ERROR_EXIT, 'Replay without an explicit --replay-status must be refused, never defaulted.');
