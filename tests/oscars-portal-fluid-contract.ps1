...
# 2b. Every tile is a 2:3 poster with the art behind the copy (3.2.62).
...
# 6b. Winners are portraits, the rotation is a marquee, the hero drifts (3.2.62).
...
Assert-Contract ($pageTemplate -match 'lunara-oscars-portal-slot-hero<\?php echo '''' !== \$hero_style \? '' has-backdrop'' : ''''; \?>"') 'The live page template must stamp has-backdrop on the hero it renders, or the drift never fires (3.2.62).'
...
# 3.2.62 raised the route ceiling from 45,000 to 57,344 bytes (56 KB): the
...
Write-Host 'Theme 3.2.62 Oscars portal fluid contract passed: one 1720px cap in three authorities, board poster wall with tile art, portrait winners, backdrop marquee, drifting hero, daily warmer, composer hook, poster-first gallery, caps removed.'
