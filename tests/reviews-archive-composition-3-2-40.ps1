...
foreach ($slot in @('hero','utility','grid','pagination','pairing-desk')) {
    Assert-True ($reviewRendering -match ('lunara-review-archive-slot-' + [regex]::Escape($slot) + '[^>]+style="order:<\?php[^>]+\$lane_orders\[''' + [regex]::Escape($slot) + '''\]')) "Rendered $slot must consume its canonical lane order."
}
...
Assert-True ($archiveCss.Contains('body.post-type-archive-review .lunara-review-archive-page')) 'Archive CSS must stay route scoped.'
...
$versionLine = (Read-ThemeFile 'style.css' | Select-String -Pattern 'Version:\s*3\.2\.40').Matches.Count
Assert-True ($versionLine -ge 1) 'Theme version must be 3.2.40.'

Write-Host 'Theme 3.2.40 Reviews Archive composition contract passed.'
