...
Assert-True ($style -match 'Version:\s*3\.2\.23') 'Theme must identify the Review visual-balance release.'
Assert-True ($front -match 'width:\s*min\(1060px,\s*calc\(100vw\s*-\s*88px\)\)') 'The desktop Debrief must use the calmer 1060px editorial measure.'
Assert-True ($front -match 'rgba\(224,\s*196,\s*129,\s*0\.09\)') 'The Debrief gradient must retain only the subtle warm-gold bloom.'
Assert-True ($front -notmatch 'rgba\(112,\s*148,\s*185,\s*0\.12\)') 'The former steel-blue bloom must not return.'
Assert-True ($front -match '0 18px 44px rgba\(0,\s*0,\s*0,\s*0\.22\)') 'The Debrief shadow must remain restrained.'
Assert-True ($front -match 'linear-gradient\(135deg,\s*rgba\(255,\s*255,\s*255,\s*0\.022\),\s*transparent 38%\)') 'The signature surface must use a quiet sheen instead of the former heavy striped emboss.'
