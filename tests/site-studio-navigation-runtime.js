...
(async () => {
 ...
 try {
  ...
  for (const width of [1440, 390]) {
   for (const [family, surface] of Object.entries(mainPages)) {
    ...
    if (surface === 'reviews-archive' || surface === 'journal-archive' || surface === 'oscars-portal') {
     ...
     equal(groups.map(group => group.id), portal ? ['essentials', 'hero-buttons', 'quick-start', 'winner-sections', 'fine-tune', 'advanced', 'revision-history'] : ['essentials', 'stories', 'gallery', 'retention', 'fine-tune', 'advanced', 'revision-history'], `${surface}: shared inspector groups without false Mobile category`);
     equal(groups.filter(group => group.open).map(group => group.id), ['essentials'], `${surface}: Content opens first`);
     equal(groups.map(group => group.label), portal ? ['Content', 'Hero buttons', 'Quick Start', 'Winner sections', 'Layout', 'Advanced', 'History'] : ['Content', 'Stories', 'Gallery', 'Continue reading', 'Layout', 'Advanced', 'History'], `${surface}: shared plain-language inspector labels`);
  ...
  for (const width of [1440, 390]) {
   ...
   assert(await page.locator('[data-studio-tool-directory]').getAttribute('open') !== null && await page.locator('[data-lunara-surface-card]:visible').count() === directorySurfaces.length, `No JS/${width}: native disclosure exposes the full directory`);
