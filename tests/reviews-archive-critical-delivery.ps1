...
Assert-True ($boostResult.registrations.Count -eq 10) 'Reviews archive, Journal archive/article, Oscars portal, and Oscars ledger must each register one concat and one async handle-scoped Boost filter.'
$expectedBoostRegistrations = @(
    ...
    @{ Hook = 'css_do_concat'; Callback = 'lunara_keep_journal_single_css_direct' },
    @{ Hook = 'jetpack_boost_async_style'; Callback = 'lunara_keep_journal_single_css_direct' }
)
...
Assert-True ($style -match '(?m)^Version:\s*3\.2\.78\s*$') 'Theme version must preserve the Reviews critical-delivery repair in 3.2.78.'
...
Write-Host "Theme 3.2.78 Reviews critical delivery contract passed: universal seed ${seedByteCount}B ($seedSha256); route CSS ${routeCssBytes}B; Boost fixture ${fixtureBytes}B."
