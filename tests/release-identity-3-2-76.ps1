...
$exactVersionHeaders = @($styleLines | Where-Object { $_ -ceq 'Version: 3.2.76' })
Assert-Contract ($versionHeaderLines.Count -eq 1 -and $exactVersionHeaders.Count -eq 1) `
    'style.css must contain exactly one exact Version: 3.2.76 header.'

$priorVersion = @('3', '2', '75') -join '.'
...
foreach ($source in $trackedSources) {
    ...
    for ($index = 0; $index -lt $sourceLines.Count; $index++) {
        ...
        # Dated public CSS evidence is a historical capture, not a runtime identity.
        $captureReference = "fixtures/reviews-published-opening-$priorVersion.json"
        if ($source.Name -ceq 'reviews-opening-browser-runtime.js' -and $line.Contains($captureReference)) {
            $line = $line.Replace($captureReference, 'historical-public-css-fixture')
        }
...
$changelogHeading = "## 2026-09-13 $releaseSeparator Theme 3.2.76 shared editors and article presentation"
...
Assert-Contract ($changelogHeadingCount -eq 1) 'The 3.2.76 changelog heading must exist exactly once.'
...
# 2026-08-31 change that removed it). What is asserted instead: the 3.2.76
...
Assert-Contract ($changelogHeadingIndex -ge 0 -and $newerThemeHeadings.Count -eq 0) `
    'The 3.2.76 changelog entry must be the newest theme release entry; only plugin-only entries may sit above it.'
$changelogEntry = Get-TopEntry -Text $changelog -Heading $changelogHeading
foreach ($term in @('Latest Reviews', 'Journal', 'Academy', 'Preview', 'History', 'mobile')) {
    Assert-Contract ($changelogEntry.Contains($term)) "The current changelog must cover $term."
}

$sessionHeading = "## 2026-09-13 $releaseSeparator Theme $priorVersion acceptance and 3.2.76 site experience"
...
Assert-Contract ($sessionHeadingCount -eq 1) 'The final 3.2.76 local-candidate session heading must exist exactly once.'
...
# its explicit did-not-occur list, and the Dalton/WordPress.com deployment
...
Assert-Contract ($sessionEntry -match '(?s)No agent deployment or manual cache purge occurred\. Theme 3\.2\.76 is not\s+deployed; its live canary is pending\.') `
...
if ($script:Failures.Count -gt 0) {
    ...
    throw "Theme 3.2.76 release identity contract failed:`n$($details -join "`n")"
}

Write-Host 'Theme 3.2.76 release identity contract passed: exact stylesheet identity, stale-version census, deploy exclusions, and intact local-only release records.'
