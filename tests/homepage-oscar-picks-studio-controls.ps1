...
Assert-True ($controlDesk -match "'lunara_home_oscar_picks_count'[\s\S]*?'min'\s*=>\s*1[\s\S]*?'max'\s*=>\s*16") 'Oscar Picks count must be clamped from 1 to 16.'
...
Assert-True ($functions -match "max\( 1, min\( 16,") 'Oscar Picks count must be clamped in the renderer.'
