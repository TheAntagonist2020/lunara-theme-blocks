...
$exactVersionHeaders = @($styleLines | Where-Object { $_ -ceq 'Version: 3.2.70' })
Assert-Contract ($versionHeaderLines.Count -eq 1 -and $exactVersionHeaders.Count -eq 1) `
    'style.css must contain exactly one exact Version: 3.2.70 header.'

$priorVersion = @('3', '2', '69') -join '.'
...
$changelogHeading = "## 2026-09-12 $releaseSeparator Theme 3.2.70 Archive Story Selection"
...
Assert-Contract ($changelogHeadingCount -eq 1) 'The 3.2.70 changelog heading must exist exactly once.'
...
# 2026-08-31 change that removed it). What is asserted instead: the 3.2.70
...
Assert-Contract ($changelogHeadingIndex -ge 0 -and $newerThemeHeadings.Count -eq 0) `
    'The 3.2.70 changelog entry must be the newest theme release entry; only plugin-only entries may sit above it.'
$changelogEntry = Get-TopEntry -Text $changelog -Heading $changelogHeading
foreach ($term in @('Preview', 'Apply', 'Home', 'Reviews', 'Journal', 'Oscars', 'mobile', 'keyboard', 'revision')) {
    Assert-Contract ($changelogEntry.Contains($term)) "The current changelog must cover $term."
}

$sessionHeading = "## 2026-09-12 $releaseSeparator Theme $priorVersion acceptance and 3.2.70 archive selection"
...
Assert-Contract ($sessionHeadingCount -eq 1) 'The final 3.2.70 local-candidate session heading must exist exactly once.'
...
Assert-Contract ($sessionEntry -match '(?s)No agent deployment or manual cache purge occurred\. Theme 3\.2\.70 is not\s+deployed; its live canary is pending\.') `
...
if ($script:Failures.Count -gt 0) {
    ...
    throw "Theme 3.2.70 release identity contract failed:`n$($details -join "`n")"
}

Write-Host 'Theme 3.2.70 release identity contract passed: exact stylesheet identity, stale-version census, deploy exclusions, and intact local-only release records.'
