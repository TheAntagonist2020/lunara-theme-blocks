...
add_filter( 'lunara_site_studio_surfaces', static function ( $surfaces ) { global $lunara_preview_surface_mutation; if ( $lunara_preview_surface_mutation && isset( $surfaces['global-design'] ) ) { if ( 'owner' === $lunara_preview_surface_mutation ) { $surfaces['global-design']['owner'] = 'theme:foreign'; } if ( 'query' === $lunara_preview_surface_mutation ) { $surfaces['global-design']['preview_query_arg'] = 'other_preview'; } if ( 'route' === $lunara_preview_surface_mutation ) { $surfaces['global-design']['preview_route'] = '/journal/'; } } return $surfaces; }, 40 );
...
function lunara_preview_record( $surface, $token, $state = null ) {
	$map = array( 'global-design' => 'theme:global-design', 'homepage-structure' => 'theme:homepage-structure', 'lunara-method' => 'theme:lunara-method' );
	$user = get_current_user_id(); $owner = $map[ $surface ];
	return array( 'user_id' => $user, 'surface' => $surface, 'owner' => $owner, 'route' => '/', 'token_hash' => wp_hash( $token . '|' . $user . '|' . $surface . '|' . $owner . '|/' ), 'expires' => current_time( 'timestamp', true ) + 600, 'state' => null === $state ? lunara_preview_state( $surface ) : $state );
}
function lunara_preview_reset( $surface, $token, $instance, $query = null, $get = null ) {
	global $lunara_test_filters, $lunara_test_actions, $lunara_preview_base_filters, $lunara_preview_base_actions, $lunara_preview_transients, $lunara_preview_events, $lunara_preview_status, $lunara_preview_user_id, $lunara_test_can_edit, $lunara_test_dependency_ready, $lunara_test_adapter_failure, $lunara_test_enqueued_scripts, $lunara_test_localized, $lunara_preview_is_front, $lunara_preview_front_id, $lunara_preview_surface_mutation, $lunara_test_trace_callbacks, $lunara_test_wp_die_args, $lunara_preview_callback_events, $lunara_preview_collision, $lunara_preview_consumer_mutation, $lunara_preview_warnings;
	$args = array( 'global-design' => 'lunara_global_design_preview', 'homepage-structure' => 'lunara_homepage_preview', 'lunara-method' => 'lunara_method_preview' );
	$lunara_test_filters = $lunara_preview_base_filters; $lunara_test_actions = $lunara_preview_base_actions;
	$lunara_preview_events = array(); $lunara_preview_status = 200; $lunara_preview_user_id = 7; $lunara_test_can_edit = true; $lunara_test_dependency_ready = true; $lunara_test_adapter_failure = ''; $lunara_test_enqueued_scripts = array(); $lunara_test_localized = array(); $lunara_preview_is_front = true; $lunara_preview_front_id = 42; $lunara_preview_surface_mutation = ''; $lunara_test_trace_callbacks = false; $lunara_test_wp_die_args = array(); $lunara_preview_callback_events = array(); $lunara_preview_collision = ''; $lunara_preview_consumer_mutation = ''; $lunara_preview_warnings = array();
	$query = null === $query ? rawurlencode( $args[ $surface ] ) . '=' . rawurlencode( $token ) . '&lunara_site_studio_instance=' . rawurlencode( $instance ) : $query;
	$_GET = null === $get ? array( $args[ $surface ] => $token, 'lunara_site_studio_instance' => $instance ) : $get;
	$_SERVER['QUERY_STRING'] = $query; $_SERVER['REQUEST_METHOD'] = 'GET'; $_SERVER['HTTP_HOST'] = 'example.test'; $_SERVER['REQUEST_URI'] = '/?' . $query; $_SERVER['HTTPS'] = 'on'; $_SERVER['SERVER_PORT'] = '443';
	$lunara_preview_transients = array( 'lunara_site_studio_preview_' . hash( 'sha256', $token ) => lunara_preview_record( $surface, $token ) );
}
...
lunara_test_assert( array( 'lunara_site_studio_handle_private_preview', 'lunara_handle_festival_qr_redirect', 'lunara_journal_archive_studio_guard_preview_request', 'lunara_reviews_archive_studio_guard_preview_request', 'lunara_oscars_portal_studio_guard_preview_request', 'lunara_send_home_cinematic_hero_preload_header', 'lunara_search_command_template_redirect' ) === $preview_callbacks && 1 === count( array_unique( array_column( $preview_actions, 'priority' ) ) ) && 0 === $preview_actions[0]['priority'], 'The public guard must be first at exact priority zero while every existing theme callback keeps its normal relative order behind it.' );
...
lunara_test_assert( $normal['ok'] && $before_filters === $lunara_test_filters && $before_scripts === $lunara_test_enqueued_scripts && $before_events === $lunara_preview_events, 'A normal Homepage request must be a total no-op.' );
$lunara_preview_callback_events = array(); $normal_dispatch = lunara_preview_dispatch(); lunara_test_assert( $normal_dispatch['ok'] && array( 'festival-qr', 'journal-guard', 'reviews-guard', 'oscars-guard', 'hero-preload', 'search-command' ) === $lunara_preview_callback_events && $before_events === $lunara_preview_events, 'A normal request must preserve every pre-existing priority-zero callback in its original executable order without private-preview effects.' );
...
foreach ( array( 'dependency-throw', 'factory-throw', 'adapter-absent', 'adapter-malformed', 'adapter-reentrant' ) as $kind ) {
	...
	$private_index = array_search( 'private', $lunara_preview_events, true ); $denied_order = false !== $private_index; foreach ( array( 'dependency', 'factory', 'transient', 'validate', 'projection' ) as $event_name ) { foreach ( array_keys( $lunara_preview_events, $event_name, true ) as $event_index ) { $denied_order = $denied_order && $event_index > $private_index; } }
	lunara_test_assert( ! $result['ok'] && 403 === $result['status'] && array( 'response' => 403 ) === $result['die_args'] && $generic === $result['message'] && array_slice( $lunara_preview_events, 0, 2 ) === array( 'headers', 'private' ) && $denied_order && array() === $lunara_preview_callback_events && array() === $lunara_test_enqueued_scripts && array() === $lunara_test_localized, $kind . ' dependency/adapter denial must be indistinguishable, private-first, live-inert, and child-inert.' );
}

foreach ( array( 'global-design', 'homepage-structure', 'lunara-method' ) as $surface ) {
	lunara_preview_reset( $surface, $token, $instance ); $canonical_before = serialize( array( $lunara_preview_options, $lunara_preview_mods, $lunara_preview_posts, $lunara_preview_transients ) ); $lunara_test_trace_callbacks = true; $accepted = lunara_preview_run();
	...
	lunara_test_assert( $keys === array( 'instance', 'markers', 'protocol', 'surface', 'type', 'version' ) && 'lunara-site-studio/v1' === $child['protocol'] && 1 === $child['version'] && 'select-section' === $child['type'] && $surface === $child['surface'] && $instance === $child['instance'] && false === strpos( wp_json_encode( $child ), $token ) && false === strpos( wp_json_encode( $child ), 'state' ), $surface . ' child config must expose only the exact six safe keys.' );
...
foreach ( $denials as $kind ) {
	...
	lunara_test_assert( ! $result['ok'] && 403 === $result['status'] && $generic === $result['message'] && $denied_order, $kind . ' must be indistinguishable and keep every reached dependency/adapter/state callback after the private boundary.' );
}
