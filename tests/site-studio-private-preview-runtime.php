...
/** Trace every real projection callback, including the five 3.2.73 surfaces. */
...
foreach ( array( 'global-design', 'homepage-structure', 'lunara-method', 'reviews-archive', 'journal-archive', 'review-single', 'utility-search', 'site-footer' ) as $surface ) {
	...
	if ( 'lunara-method' === $surface ) {
		lunara_test_assert( 0 === get_theme_mod( 'lunara_home_pairing_desk_review_id', 99 ) && 44 === get_theme_mod( 'lunara_home_pairing_desk_backdrop_id', 0 ) && '' === get_theme_mod( 'lunara_home_pairing_desk_kicker', 'fallback' ), 'Method preview must preserve exact zero/empty candidate values in memory.' );
		$framed = lunara_preview_state( $surface ); $framed['review_mode'] = 'manual'; $framed['review_id'] = 999; $framed['backdrop'] = array( 'hidden' => true, 'focal_x' => 17, 'focal_y' => 81, 'fit' => 'full', 'zoom' => 109 );
		lunara_test_assert( lunara_site_studio_preview_install_state( $surface, $framed, 42 ) && 'manual' === get_theme_mod( 'lunara_home_pairing_desk_review_mode' ) && 999 === get_theme_mod( 'lunara_home_pairing_desk_review_id' ) && $framed['backdrop'] === lunara_method_backdrop_settings(), 'Private Method preview must feed the exact same mode/framing read path used by the active public renderer.' );
		lunara_test_assert( $canonical_before === serialize( array( $lunara_preview_options, $lunara_preview_mods, $lunara_preview_posts, $lunara_preview_transients ) ), 'Method framing preview never persists its request-local overrides.' );
	}
