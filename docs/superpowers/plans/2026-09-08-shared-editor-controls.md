...
- [ ] Add focused resolver/metadata regression cases for review artwork fallback,
  explicit image-off behavior, manual override, unavailable selections, and automatic order.
- [ ] Resolve inherited artwork through canonical Review Image Studio/Journal
  accessors; expose the same resolved image to editor and public delivery.
- [ ] Extend the generic controller with the adapter interface and exact carousel
...
- [ ] Use Preview changes / Apply changes / Discard changes consistently. Show a
  neutral loading state until initialization completes, and useful failure feedback.
- [ ] Run focused PHP and generic Site Studio tests; report and commit owned files.
...
- [ ] Replace the independent carousel save controller with the Task 1 adapter.
- [ ] Render searchable story thumbnails, dates, selected state, accessible Move
  up/down and drag ordering, contextual item removal, and unavailable-item warnings.
- [ ] Show automatic lineup read-only and offer Use this lineup in Manual. Never
  replace an existing manual list without an explicit action.
- [ ] Add image thumbnails, Media Library replacement, source reset, destination
...
- [ ] Make discarded/restored items reject stale media/search responses. Keep
  focus attached to moved items and retain selection on request failures.
- [ ] Verify two independent modes, keyboard and pointer ordering, image/copy
...
- [ ] Review the integrated implementation against the editor standard.
- [ ] Run the full required theme suite, focused mutations, syntax and diff checks.
- [ ] Verify rendered editor controls in a browser using the local fixture; record
  any live-only acceptance that awaits Dalton's deployment.
- [ ] Advance release identity to 3.2.63, update guide/changelog/session log, commit
